    ...
    for target_index in range(len(target_word_pair)):
        ...
        for seed in random_seeds:    
            ...
            if train_mode == "unified":
                ...
                x_train,y_train,x_train_target,x_train_target_set = pp.clean_all(filename1,col,dataset_name,normalization_dict)
                x_val,y_val,x_val_target,x_val_target_set = pp.clean_all(filename2,col,dataset_name,normalization_dict)
                x_test,y_test,x_test_target,x_test_target_set = pp.clean_all(filename3,col,dataset_name,normalization_dict)
            ...
            for fix in range(4):
                if fix != 3:
                    for i, tar in enumerate(x_train_target):
                        x_train_target[i] = x_train_target_set[tar][fix]
                    for i, tar in enumerate(x_val_target):
                        x_val_target[i] = x_val_target_set[tar][fix]
                    for i, tar in enumerate(x_test_target):
                        x_test_target[i] = x_test_target_set[tar][fix]
