...
#                 y_train2 = torch.load('/content/DI-MTSD/teacher_models/different_test_dataset/corrected_code/teacher_output_all_batch'+'_seed{}.pt'.format(seed))
            ...
            unique_targets_test = [['donald', 'trump'],['21','Stop','That','century','fox','and','disney'],['joe','biden']]
            related_terms_test = [[['bush'],['hillary'],['trump#trump']],[['disneydeal'],['disneyand'],['disneyworld']],[['bidens'],['biddi'],['joe','biden']]]
            
            for fix in range(4):        
              x_train_target_2 = x_train_target.copy()
              x_val_target_2 = x_val_target.copy()
              x_test_target_2 = x_test_target.copy()      
              if fix != 3:              
                for i in range(len(x_train_target)):
                  index = unique_targets.index(x_train_target[i])
                  x_train_target_2[i] = related_terms[index][fix]
                for i in range(len(x_val_target)):
                  index = unique_targets.index(x_val_target[i])
                  x_val_target_2[i] = related_terms[index][fix]
              ...
              x_train_all = [x_train,y_train,x_train_target_2]
              x_val_all = [x_val,y_val,x_val_target_2]
              x_test_all = [x_test,y_test,x_test_target_2]
