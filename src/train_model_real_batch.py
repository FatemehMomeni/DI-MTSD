...
def run_classifier():
    ...
    target_num = {'mt': 6, 'semeval': 4, 'wtwt': 4, 'am': 8, 'covid': 1, 'all': 3}  # 'all': 3 (generalization), 4 (srq)
    ...
    for target_index in range(len(target_word_pair)):
        ...
        for seed in random_seeds:    
            ...
            for epoch in range(0, total_epoch):
                ...
                if model_name == 'teacher':
                  for z in range(4):                    
                    for input_ids,seg_ids,atten_masks,target,length in trainloader[z]:
                        optimizer.zero_grad()
                  ...
                  for z in range(4):
                    for input_ids,seg_ids,atten_masks,target,length,target2 in trainloader[z]:
                        optimizer.zero_grad()
                        output1 = model(input_ids, seg_ids, atten_masks, length)
                        output2 = output1
                ...
                if model_name == 'teacher':
                    ...
                    with torch.no_grad():
                      for z in range(4):
                        for input_ids,seg_ids,atten_masks,target,length in trainloader_distill[z]:
                            output1 = model(input_ids, seg_ids, atten_masks, length)
                ...
                with torch.no_grad():
                    if not eval_batch[dataset_name]:
                      ...
                      for z in range(4):
                        for input_ids,seg_ids,atten_masks,target,length in valloader[z]:
                ...
                x_test_len_all = list()                
                x_test_input_ids_all = list()
                x_test_seg_ids_all = list()
                x_test_atten_masks_all = list()
                for z in range(4):
                  x_test_len_all.extend(x_test_len[z])                  
                  x_test_input_ids_all.extend(x_test_input_ids[z])
                  x_test_seg_ids_all.extend(x_test_seg_ids[z])
                  x_test_atten_masks_all.extend(x_test_atten_masks[z])

                if train_mode == "unified":
                    x_test_len_list = dh.sep_test_set(x_test_len_all,dataset_name)
                    y_test_list = dh.sep_test_set(y_test,dataset_name)
                    x_test_input_ids_list = dh.sep_test_set(x_test_input_ids_all,dataset_name)
                    x_test_seg_ids_list = dh.sep_test_set(x_test_seg_ids_all,dataset_name)
                    x_test_atten_masks_list = dh.sep_test_set(x_test_atten_masks_all,dataset_name)
                
                with torch.no_grad():
                    if eval_batch[dataset_name]:
                        test_preds = []
                        for z in range(4):
                          for input_ids,seg_ids,atten_masks,target,length in testloader[z]:
