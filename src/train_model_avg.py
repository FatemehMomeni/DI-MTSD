...
import argparse
import json
import utils.data_helper_avg as dh
...
def run_classifier():
  ...
  for seed in random_seeds:
    ...
    for epoch in range(total_epoch):
      ...
      for input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, input_ids_rel_tar1, atten_mask_rel_tar1, \
          input_ids_rel_tar2, atten_mask_rel_tar2, input_ids_rel_tar3, atten_mask_rel_tar3, y in train_loader:           
        tar_embeds, txt_embeds = model(input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, None)        
        rel_tar_embeds1, rel_tar_embeds2 = model(input_ids_rel_tar1, atten_mask_rel_tar1, input_ids_rel_tar2, atten_mask_rel_tar2, None)
        rel_tar_embeds3, _ = model(input_ids_rel_tar3, atten_mask_rel_tar3, None, None, None)
        mean_embeds = torch.stack((rel_tar_embeds1, rel_tar_embeds2, rel_tar_embeds3)).mean(dim=0)
        embeds = torch.cat((tar_embeds, mean_embeds, txt_embeds), 1)
        optimizer.zero_grad()
      ...
      with torch.no_grad():
        for input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, input_ids_rel_tar1, atten_mask_rel_tar1, \
            input_ids_rel_tar2, atten_mask_rel_tar2, input_ids_rel_tar3, atten_mask_rel_tar3, y in train_loader_distil:
          tar_embeds, txt_embeds = model(input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, None)          
          rel_tar_embeds1, rel_tar_embeds2 = model(input_ids_rel_tar1, atten_mask_rel_tar1, input_ids_rel_tar2, atten_mask_rel_tar2, None)
          rel_tar_embeds3, _ = model(input_ids_rel_tar3, atten_mask_rel_tar3, None, None, None)
          mean_embeds = torch.stack((rel_tar_embeds1, rel_tar_embeds2, rel_tar_embeds3)).mean(dim=0)
      ...
      with torch.no_grad():
        for input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, input_ids_rel_tar1, atten_mask_rel_tar1, \
            input_ids_rel_tar2, atten_mask_rel_tar2, input_ids_rel_tar3, atten_mask_rel_tar3, y in val_loader:
          tar_embeds, txt_embeds = model(input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, None)          
          rel_tar_embeds1, rel_tar_embeds2 = model(input_ids_rel_tar1, atten_mask_rel_tar1, input_ids_rel_tar2, atten_mask_rel_tar2, None)
          rel_tar_embeds3, _ = model(input_ids_rel_tar3, atten_mask_rel_tar3, None, None, None)
          mean_embeds = torch.stack((rel_tar_embeds1, rel_tar_embeds2, rel_tar_embeds3)).mean(dim=0)
      ...
      with torch.no_grad():
        test_preds = []
        for input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, input_ids_rel_tar1, atten_mask_rel_tar1, \
            input_ids_rel_tar2, atten_mask_rel_tar2, input_ids_rel_tar3, atten_mask_rel_tar3, y in test_loader:
          tar_embeds, txt_embeds = model(input_ids_tar, atten_mask_tar, input_ids_txt, atten_mask_txt, None)          
          rel_tar_embeds1, rel_tar_embeds2 = model(input_ids_rel_tar1, atten_mask_rel_tar1, input_ids_rel_tar2, atten_mask_rel_tar2, None)
          rel_tar_embeds3, _ = model(input_ids_rel_tar3, atten_mask_rel_tar3, None, None, None)
          mean_embeds = torch.stack((rel_tar_embeds1, rel_tar_embeds2, rel_tar_embeds3)).mean(dim=0)
