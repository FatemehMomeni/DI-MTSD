...
def data_helper_bert(data, batch_size, mode):
  ...
  encoded_rel_tar1 = tokenizer(data[3], max_length=128, padding='max_length', truncation=True)    
  encoded_rel_tar2 = tokenizer(data[4], max_length=128, padding='max_length', truncation=True)
  encoded_rel_tar3 = tokenizer(data[5], max_length=128, padding='max_length', truncation=True)    
  ...
  input_ids_rel_tar1 = torch.tensor(encoded_rel_tar1['input_ids']).cuda()
  atten_masks_rel_tar1 = torch.tensor(encoded_rel_tar1['attention_mask']).cuda()
  input_ids_rel_tar2 = torch.tensor(encoded_rel_tar2['input_ids']).cuda()
  atten_masks_rel_tar2 = torch.tensor(encoded_rel_tar2['attention_mask']).cuda()
  input_ids_rel_tar3 = torch.tensor(encoded_rel_tar3['input_ids']).cuda()
  atten_masks_rel_tar3 = torch.tensor(encoded_rel_tar3['attention_mask']).cuda()      
  y = torch.tensor(data[2]).cuda()
  
  tensor_loader = TensorDataset(input_ids_tar, atten_masks_tar, input_ids_txt, atten_masks_txt, \
                                input_ids_rel_tar1, atten_masks_rel_tar1, input_ids_rel_tar2, atten_masks_rel_tar2,\
                                input_ids_rel_tar3, atten_masks_rel_tar3, y)
