...
def data_helper_bert(data, batch_size, mode):
  ...
  new_targets = list()
  for tar in range(len(data[0])):
    relateds = data[3][tar] + ' ' + data[4][tar] + ' ' + data[5][tar]
    new_targets.append(data[0][tar] + ' [SEP] ' + relateds)
  
  encoded_tar = tokenizer(new_targets, max_length=128, padding='max_length', truncation=True)    
